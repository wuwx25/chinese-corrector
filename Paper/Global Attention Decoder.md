...
在以上公式中，$a^{p,q}_{i,j}$代表的第p个字符的第q个候选对于第i个字符的第j个候选的权重大小，$W_g^V$则是一个可训练的矩阵。通过以上的公式，GAD输出得出的是第i个字符的第j个候选的全局上下文表示$GA_{i,j}$，其中包含了其余潜在正确字符以及潜在错误字符的候选信息。然后将字符i的所有候选按权重相加，就能得到第i个字符的表示信息$GA_i$，过程如下所示：

<center>
    <img style="border-radius: 0.3125em;
    box-shadow: 0 2px 4px 0 rgba(34,36,38,.12),0 2px 10px 0 rgba(34,36,38,.08);" 
    src="../docs/GAD_4.png" width = "50%" alt=""/>
    <br>
    <div style="color:orange; border-bottom: 1px solid #d9d9d9;
    display: inline-block;
    color: #999;
...
最后的一步自然就是根据表示$GA$来生成概率分布，从而预测该位置正确的字符。

#### 总结和展望
在今年的几篇关于CSC的论文中，GAD已经算是比较创新的了，针对于局部上下文的错误信息提出了一种新的框架，虽然说牺牲了一些时间复杂度和空间复杂度（attention的权重参数增多），并且对于模型是否真的能够在庞大的上下文领域中学习到正确的有用的信息也是比较玄学的，毕竟之前的上下文表示只学习了原句子中字符之间的关系，本篇文章中学习的却是字符的候选列表之间的关系。但终究是从0到1的突破，所以这个思想看起来还是有一些可取之处的，未来的一些工作也可以围绕如何在上下文领域中学习到更为准确的信息来进行，也就是关于global Attention这一策略的改进。可惜的是这篇文章并没有公开的官方代码，所以也没有办法看到具体的效果如何。之后如果作者进行了开源，也可以看看是否真的能够解决局部误导信息的问题。
